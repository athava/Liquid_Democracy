...
function form2data(data) {
    ...
    data = dformat(data, 'password', md5(document.getElementById('pw').value));
...
function delete_inititative(initiative_id) {
    var data = '';
    data = dformat(data, 'initiative_id', initiative_id);
    data = dformat(data, 'action', 'delete_initiative');

    ajax_req('POST', 'add_initiative', data);
}


function pi_page(pi_id){
    var data = '';
    data = dformat(data, 'initiative_id', pi_id);
    
    ajax_req('POST', 'pi_page.jsp', data);
}

function user_pi_list(uname, status){
    var data = '';
    data = dformat(data, 'user', uname);
    data = dformat(data, 'status', status);
    
    ajax_req('POST', 'user_pi_list', data);
}

function add_delegator(pi_id, username){
    ...
    var data = '';
    data = dformat(data, 'username', username);
    data = dformat(data, 'delegator', delagator);
    data = dformat(data, 'initiative_id', pi_id);
    data = dformat(data, 'action', 'add');
    
    ajax_req('POST', 'delegator_actions', data);
}